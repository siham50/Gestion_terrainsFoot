        ...
        if (($current_time - $last_sent) < 30) {
            ...
            if (mysqli_query($conn, $update_code)) {
                ...
                try {
                    ...
                    $mailer->isHTML(true);
                    $mailer->Subject = 'Nouveau code de vérification - Nod Tasskhoun';
                    $mailer->Body = "<p>Votre nouveau code de vérification est :</p>
                    ...
                    $mailer->AltBody = "Votre nouveau code de vérification est : $new_code\n\nCe code expire dans 5 minutes.";
                    
                    $mailer->send();
                    $_SESSION['code_sent_time'] = time();
                    $message = "<div class='success'>✅ Nouveau code envoyé ! Vérifiez votre email.</div>";
                } catch (Exception $e) {
                    $message = "<div class='error'>❌ Erreur lors de l'envoi de l'email : " . htmlspecialchars($e->getMessage()) . "</div>";
                }
            } else {
